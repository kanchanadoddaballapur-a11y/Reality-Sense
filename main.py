...
def analyze_multimodal(content_parts):
    ...
    CRITICAL WATERMARK CHECK: strictly scan the edges and corners for any watermarks, logos, or text indicating an AI generation tool.
    
    CRITICAL ANALYSIS GUIDELINES:
    If it is an image: Look for AI artifacts like unnatural textures, extra fingers, warped details, missing reflections, perfect symmetry, or deepfake patterns.
    If it is a video: Analyze the ENTIRE duration. Pay strict attention to temporal inconsistencies, morphing objects, unnatural physics (e.g., objects passing through each other), flickering textures, weird background distortions, and lack of object permanence across frames. This is a common flaw in Sora, Runway, and Pika videos.
    If it is text/document: Look for linguistic patterns, extreme perplexity, lack of burstiness, or overly generic structures common in LLMs.
